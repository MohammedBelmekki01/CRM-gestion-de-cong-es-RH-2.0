...
export async function POST(req: NextRequest) {
  ...
  if (!name || !departmentId) {
    return NextResponse.json({ error: "Nom et departmentId requis" }, { status: 400 });
  }

  const existing = await prisma.position.findFirst({ where: { name } });
  if (existing) {
    return NextResponse.json({ error: "Le poste existe déjà" }, { status: 400 });
  }
...
export async function DELETE(req: NextRequest) {
  ...
  if (!existing) return NextResponse.json({ error: "Poste non trouvé" }, { status: 404 });
